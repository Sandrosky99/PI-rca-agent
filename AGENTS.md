# Developer Context — PI RCA Workflow

> Fichero de contexto para asistentes de IA, neutral respecto al proveedor. `CLAUDE.md` es solo
> un import de este fichero, para la convención de auto-carga de Claude Code.
>
> **Este fichero es para desarrollo. NO guardar aquí secretos, credenciales ni configuración de
> ejecución** (base/software-spec §5).
...
**Propietario:** Sandrosky99 &lt;sandracerveron@gmail.com&gt; — responsable final del recurso, con
independencia de que su desarrollo se haya asistido con IA (base §1.3, §6.2).

**Clasificación de riesgo: BAJA** (ai-governance §6.1). No actúa sobre la planta, no trata datos
personales y un ingeniero lee siempre el diagnóstico antes de actuar. Ver
[`docs/AI-GOVERNANCE.md`](./docs/AI-GOVERNANCE.md) para la justificación completa y el inventario.

⚠️ **No es un producto oficial de Schneider Electric.** Ver [`DISCLAIMER.md`](./DISCLAIMER.md).
...
Cosas que un asistente de IA (o cualquier contribuidor) debe evitar en este repositorio.

- **Nunca commitear secretos.** `.env` tiene las claves de API en texto plano y está en
  `.gitignore`. Tampoco deben aparecer en `AGENTS.md`, en el log ni en los prompts
  (base §5, ai-governance §1.1).
- **Nunca commitear `webhook.log` ni `incidents/`.** Contienen payloads reales, prompts completos
  y series de datos de planta. Ambos están en `.gitignore`.
- **La salida del modelo es entrada no confiable.** Todo lo que devuelve el LLM se valida antes de
  usarse: los `piApiPath` contra el `af_context` (`_validate_selected_variables`) y la petición de
  ventana contra el catálogo (`_parse_history_request`). No añadir rutas por las que la respuesta
  del modelo llegue sin validar a una consulta, un comando o un fichero (ai-governance §1.7, §1.8).
- **No dar tools al modelo.** El diseño es deliberadamente un workflow: quien llama a los MCP
  servers es el código. Convertirlo en un agente es una decisión de arquitectura, no un ajuste.
- **Ojo con lo que se envía al LLM.** Los prompts llevan estructura de planta y datos de proceso a
  un modelo en la nube. No añadir campos nuevos al prompt sin valorar qué se está exponiendo
  (ai-governance §1.4).
...
- **Sin hardcodear credenciales ni configuración** — todo vía `.env` (base §3)
- **Documentar el porqué, no el qué** — ver más abajo
- **Commits: divulgación de autoría asistida por IA (base §1.4, MUST).** Todo commit generado
  total o parcialmente con IA debe declarar **modelo/herramienta con versión** y **fecha**:

  ```
  <asunto en español>

  <cuerpo: qué se vio fallar y por qué se resuelve así>

  AI-assisted: claude-opus-5 via Claude Code, 2026-09-07
  Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>
  ```

  El `Co-Authored-By` solo no basta: no lleva fecha. Ambos trailers juntos sí cumplen.
  Los commits anteriores al 2026-09-07 llevan solo `Co-Authored-By`; **no se reescriben**, porque
  los registros de trazabilidad no deben editarse retroactivamente (base §1.7). La brecha queda
  anotada en [`docs/AI-TRACEABILITY.md`](./docs/AI-TRACEABILITY.md).
...
Un contribuidor nuevo necesita, por este orden:

1. `README.md` — qué hace el sistema y cómo levantarlo.
2. La sección **Terminología** de arriba — por qué esto es un workflow y no un agente. Sin eso se
   busca un bucle de decisión que no existe.
3. **Detalle de cada step** — el porqué de cada guardarraíl. Casi todo lo que tiene una forma rara
   en este código está ahí porque algo falló contra el sistema real.
4. `docs/AI-GOVERNANCE.md` — clasificación de riesgo, límites y usos restringidos.
5. `tests/` — `python tests/run_all.py` sin necesidad de PI ni de claves de API.

No hace falta acceso a PI para desarrollar: las pruebas usan dobles.
...
5. `docs/AI-TRACEABILITY.md` si se ha usado un modelo o herramienta distinta
...
- izSpecs version: `2026.12-09011521`
- Validated against:
  - base/software-spec.md (v1.10)
  - extensions/ai-governance-spec.md (v1.2)
  - extensions/observability-spec.md (v1.1)

`extensions/mcp-server-spec.md` — **N/A**: este recurso es un *cliente* MCP, no un servidor; no
expone tools ni acepta conexiones MCP entrantes.

La matriz completa sección por sección, con las justificaciones de lo marcado N/A y el registro de
excepciones, está en [`docs/IZSPECS-CONFORMANCE.md`](./docs/IZSPECS-CONFORMANCE.md).
