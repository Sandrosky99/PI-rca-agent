...
| 5 | Línea base de seguridad [SHOULD] | ✅ | Secretos solo en `.env`, en `.gitignore`, con el riesgo declarado en el `README.md`. Dependencias fijadas a versión exacta y escaneadas con `pip-audit` en CI. Los errores no vuelcan trazas ni rutas internas (`observability.JsonFormatter`) |
...
| 2.1 | Cobertura de pruebas [MUST] | ✅ | 5 suites en `tests/`, ~140 comprobaciones. Umbral del proyecto: toda función con lógica de decisión o validación debe tener pruebas de su camino feliz, sus modos de fallo y sus entradas malformadas |
...
| 2.3 | Validación funcional documentada [MUST] | ⚠️ Parcial | `AI-TRACEABILITY.md` §8. Hay validación contra PI real, pero de **un solo** ciclo completo. Insuficiente para afirmar calidad de diagnóstico; declarado como límite conocido |
